...
describe("getPaths Tests", () => {
  beforeAll(() => {
    ...
    mkdirSync(join(testDir, "deep", "very", "nested", "structure"), {
      recursive: true,
    });
    writeFileSync(
      join(testDir, "deep", "very", "nested", "structure", "deep-file.txt"),
      "content",
    );
  ...
  describe("Edge cases", () => {
    ...
    it("relative paths should contain expected file and directory names", () => {
      ...
      expect(files.some((path) => path === "src/components/Button.tsx")).toBe(
        true,
      );
  ...
  describe("Advanced Glob Patterns", () => {
    ...
    it("should work with case sensitive patterns", () => {
      ...
      expect(upperFiles.some((path) => path.includes("UPPERCASE.TXT"))).toBe(
        true,
      );
    ...
    it("should work with deep nested patterns", () => {
      ...
      expect(deepFiles.some((path) => path.includes("deep-file.txt"))).toBe(
        true,
      );
  ...
  describe("Special File Names", () => {
    it("should handle files with spaces", () => {
      const files = getFilePaths(testDir, "**/*");
      expect(files.some((path) => path.includes("file with spaces.txt"))).toBe(
        true,
      );

      const specific = getFilePaths(testDir, "*with*");
      expect(
        specific.some((path) => path.includes("file with spaces.txt")),
      ).toBe(true);
    });

    it("should handle files without extensions", () => {
      const files = getFilePaths(testDir, "**/*");
      expect(files.some((path) => path.includes("file-no-extension"))).toBe(
        true,
      );

      const withExt = getFilePaths(testDir, "*.txt");
      expect(withExt.some((path) => path.includes("file-no-extension"))).toBe(
        false,
      );
    });
    ...
    it("should handle files with unicode characters", () => {
      ...
      const jsFiles = getFilePaths(testDir, "*.js");
      expect(jsFiles.some((path) => path.includes("números-ñ-ü.js"))).toBe(
        true,
      );
    });
    ...
    it("should handle various file extensions", () => {
      ...
      expect(
        getFilePaths(testDir, "*.css").some((path) =>
          path.includes("styles.css"),
        ),
      ).toBe(true);
      expect(
        getFilePaths(testDir, "*.jsx").some((path) =>
          path.includes("script.jsx"),
        ),
      ).toBe(true);
    });
